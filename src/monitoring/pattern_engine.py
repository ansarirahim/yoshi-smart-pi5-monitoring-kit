...
@brief      Analyzes sensor events to detect patterns and anomalies.
@details    Provides intelligent pattern recognition for:
...
class PatternEngine:
    ...
    def _analyze_event(self, event: SensorEvent) -> Optional[Alert]:
            ...
            if self.config.vibration_without_door_alert:
                if not self._recent_door_activity():
                    return Alert(
                        ...
                        message="⚠️ Vibration detected without door activity!",
        ...
        if event.event_type == EventType.DOOR_OPENED:
            ...
            emoji = "🚪🌙" if self._is_night_time() else "🚪"
            return Alert(
                level=level,
    ...
    def check_temperature(self, temp: float, humidity: float) -> Optional[Alert]:
        ...
        if temp > self.config.temp_high:
            return Alert(
                level=AlertLevel.WARNING,
                message=f"🌡️ High temperature: {temp:.1f}°C",
        ...
        if temp < self.config.temp_low:
            return Alert(
                level=AlertLevel.WARNING,
                message=f"❄️ Low temperature: {temp:.1f}°C",
        ...
        if humidity < self.config.humidity_low:
            return Alert(
                level=AlertLevel.INFO,
                message=f"🏜️ Low humidity: {humidity:.1f}%",
